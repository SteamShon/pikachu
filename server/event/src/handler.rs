...
use crate::{
    message_send_processor::MessageSendProcessor,
    stat_processor::StatProcessor,
    util::{init_consumer, init_future_producer},
};
...
pub fn initialize_stat_processor_runner(
    ...
    let processor = Box::new(StatProcessor {
        client: client.clone(),
        producer: producer_arc.clone(),
    });
...
pub async fn initialize_message_send_processor_runner(
    ...
    let producer_arc = Arc::new(producer);
    let processor = Box::new(MessageSendProcessor::new(client.clone()).await);
