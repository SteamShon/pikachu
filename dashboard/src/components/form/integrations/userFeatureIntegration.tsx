...
import { generateTransformCubeSql } from "../../../utils/providers/awsS3DuckDB";
...
function UserFeatureIntegration({
  ...
  const upload = async () => {
    ...
    const transformSql = await generateTransformCubeSql({
      cubeProviderDetails: cubeIntegration?.provider.details,
    ...
    try {
      ...
      await axios.post(`/api/provider/awsS3DuckDB`, {
        ...
        provider: cubeIntegration?.provider,
        payload: {
          sql: transformSql,
        },
      });
      ...
      const result = await axios.post(`/api/integration/db`, {
        ...
        method: "upload",
        provider,
        cubeProvider: cubeIntegration?.provider,
      });
      setErrorMessage(undefined);
      setValue("status", "PUBLISHED");
