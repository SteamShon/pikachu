...
impl LocalCreativeFetcher {
    pub fn ad_group_ids_to_creatives<'a: 'b, 'b>(
        ad_group_ids: &'b HashSet<String>,
        ad_group_creatives: &'a HashMap<String, HashMap<String, creative::Data>>
    ) -> HashMap<String, &'a HashMap<String, creative::Data>> {
        let mut aggr = HashMap::new();
        for ad_group_id in ad_group_ids {
            if let Some(creatives) = ad_group_creatives.get(ad_group_id) {
                aggr.insert(ad_group_id.clone(), creatives);                
            }
        }
        aggr
    }
