...
use common::db::{integration, placement, provider};
...
pub struct MessageSendProcessor {
    ...
    pub integrations: Integrations, // pub producer: Arc<Option<FutureProducer>>,
}
...
impl MessageSendProcessor {
    ...
    async fn fetch_placements(client: Arc<PrismaClient>) -> Vec<placement::Data> {
        client
            .placement()
            .find_many(vec![])
    ...
    async fn fetch_providers(client: Arc<PrismaClient>) -> Vec<provider::Data> {
        client
            .provider()
            .find_many(vec![])
...
impl Processor for MessageSendProcessor {
    async fn process(&self, messages: &Vec<BorrowedMessage<'_>>) -> Vec<bool> {
        ...
        for (placement_id, requests) in placement_requests {
            ...
            let response = self
                .integrations
                .send_sms(placement_id.as_str(), &payload)
                .await;
